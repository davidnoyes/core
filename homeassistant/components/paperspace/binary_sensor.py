"""Support for monitoring the state of a Paperspace machine."""
...
def setup_platform(
    ...
    """Set up the Paperspace machine sensor."""
    if not (paperspace := hass.data.get(DATA_PAPERSPACE)):
        return

    dev = []
    for machine in paperspace.machines:
        dev.append(PaperspaceBinarySensor(paperspace, machine))

    add_entities(dev, True)
    return


class PaperspaceBinarySensor(BinarySensorEntity):
    """Representation of a Paperspace machine sensor."""

    _attr_attribution = ATTRIBUTION

    def __init__(self, paperspace, machine):  # pylint: disable=invalid-name
        """Initialize a new Paperspace machine sensor."""
    ...
    def update(self) -> None:
        ...
        _LOGGER.debug("UPDATING PAPERSPACE MACHINE SENSOR")
